...
exports.chapaPayment = async (amount, booking, user) => {
  if (MOCK_CHAPA) {
    logger.info(`🔧 MOCK CHAPA: Payment initiated for booking ${booking._id}`);
    return {
  ...
  try {
    const baseBackend = process.env.BACKEND_URL || "http://localhost:5000";
    ...
    const payload = {
      ...
      callback_url: `${baseBackend}/api/v1/payments/webhook/chapa`,
      return_url: `${baseFrontend}/payment-success?bookingId=${booking._id}`,
      customization: {
        title: "Car Rental",
