...
exports.getPendingVerifications = async (req, res, next) => {
  try {
    const users = await User.find({
      ...
      "name email phone driverLicense driverLicenseImage idNumber idImage address profilePicture createdAt",
    );
    res.json(users);
  } catch (error) {
    next(error);
...
exports.approveVerification = async (req, res, next) => {
  try {
    ...
    if (user.verificationStatus !== "pending") {
      return res
        .status(400)
        .json({ message: "User is not pending verification." });
    }
    user.verificationStatus = "approved";
    user.verificationMessage = "Verified by admin.";
    await user.save();
    ...
    await createNotification(
      ...
      "Your account has been verified. You can now book cars!",
    );
...
exports.rejectVerification = async (req, res, next) => {
  try {
    ...
    user.verificationMessage =
      reason ||
      "Verification rejected. Please update your information and resubmit.";
    await user.save();

    await createNotification(
      user._id,
      "Verification Rejected",
      `Your verification was rejected: ${user.verificationMessage}`,
    );
